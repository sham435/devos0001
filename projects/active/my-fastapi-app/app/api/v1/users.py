...
from app.schemas.user import UserRead, UserUpdate, Page
...
async def update_me(update: UserUpdate, db: AsyncSession = Depends(get_db),
    ...
    return await user_service.update_user(db, current_user, update)
...
async def get_users(skip: int = 0, limit: int = 20, db: AsyncSession = Depends(get_db),
    ...
    return Page(items=[UserRead.model_validate(u) for u in users], total=total,
